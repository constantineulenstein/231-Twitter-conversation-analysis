...
def create_tree(conversation_id, conversation_data):
    ...
    print(
        f"Tree size is {tree.size()}, tree depth is {tree.depth()} and tree width is {len(depth_1_nodes)}."
    )
...
def create_tree_with_networkx(conversation_id, conversation_data):
    ...
    for tree_datapoint in conversation_data:
        try:
            G.add_edge(tree_datapoint[0], tree_datapoint[1])
        except:
            error_count += 1
    print(
        f"Created Tree with {len(conversation_data) - error_count} replies. {error_count} replies were throwing "
    ...
    G0 = G.subgraph(Gcc[0])
    structural_virality = nx.wiener_index(G0) / (
    ...
    pos = nx.nx_agraph.graphviz_layout(G0, prog="dot", root=conversation_id)
    nx.draw(G0, pos, node_size=3)
    plt.savefig(f"graph_{conversation_id}.png")
...
def create_graph(conversation_id, conversation_data, client):
    ...
    og_author_id = client.get_tweet(
        conversation_id, expansions=["author_id"]
    ).data.author_id
    ...
    for graph_datapoint in conversation_data:
        try:
            if graph_datapoint[0] in tweet_to_author_dict:
                ...
                reply = client.get_tweet(
                    graph_datapoint[0], expansions=["author_id"]
                ).data.author_id
